...
先将 `<内容目录绝对路径>` 和 `<访问根地址>` 替换为实际部署值，例如 `/home/user_00/html-preview/` 和 `http://192.168.15.45:8088`。地址应从用户浏览器可达；这些示例不是所有安装环境的默认值。
...
例如文件名为 `checkout-20260911-103000.html`，审查链接为 `http://192.168.15.45:8088/_review?page=%2Fcheckout-20260911-103000.html`。文件复制到内容目录后即由现有服务提供，无需重启。HTTP 检查用于确认发布可达，首次安装的交互验收见 [INSTALL.md](INSTALL.md)。
