...
2. 浏览器打开首页，分别进入 Markdown 和 HTML；确认正文、Markdown 表格、代码块与 Mermaid 流程图正常展示。
...
按 `README.md` 的「让 Agent 自动发布方案」生成指令，将 `<内容目录绝对路径>` 和 `<访问根地址>` 替换为本次安装的值。需要留在项目里的方案写在项目目录，用 `/_fs` 绝对路径审查；临时独立方案发布到内容目录。
...
简洁报告：安装位置、可访问 URL、临时方案的内容目录、项目方案可留在项目目录并用绝对路径审查、实际启动与停止命令、已通过和未完成的验收项。提醒批注保存在当前浏览器，分享链接只分享方案入口；保留访问域名与端口有助于继续使用原批注。
